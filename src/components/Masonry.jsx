...
const Masonry = ({
  ...
  const grid = useMemo(() => {
    ...
    const mapped = items.map((child) => {
      ...
      const height = child.height / 2; // skala tinggi gambar
    ...
    return {
      items: mapped,
      containerHeight: Math.max(...colHeights), // ambil tinggi kolom tertinggi
    };
  }, [columns, items, width]);
  ...
  return (
    <div
      ...
      style={{ height: grid.containerHeight || "auto" }} // ✅ fix tinggi container
    >
      {grid.items.map((item) => (
        <div
          key={item.id}
